...
class Models(object):
    ...
    def fit(self, X, y):
        ...
        :param X: The input array (2 dimensions)
        :param y: The input target (1 dimension)
...
class CrossValidatedModel(object):
    def __init__(self, model, n_splits=3):
        ...
        self.models = [clone(model) for _ in range(n_splits)]
    ...
    def predict_proba(self, X):
        predictions = np.full(
            (len(X), len(self.models[0].classes_), self.n_splits),
            np.nan
        )
